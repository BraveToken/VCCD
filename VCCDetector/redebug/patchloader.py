...
class PatchLoader(object):
    ...
    def _process(self, patch_path):
        ...
        '''
        patch_filename = re.split('/|\\\\', patch_path)[-1]
        ...
        for line in patch_lines:
            if line.startswith('--- '):
                if diff_vuln_lines:
                    diff_norm_lines = self._normalize(
                        ''.join(diff_vuln_lines), magic_ext).split()
                    if len(diff_norm_lines) >= common.ngram_size:
                        ...
                        self._patch_list.append(common.PatchInfo(
                            path, magic_ext, ''.join(diff_orig_lines), diff_norm_lines, hash_list))
                    else:
                        common.verbose_print('      %s %d (ext: %d) - skipped (%d lines)' % (
                            diff_file, diff_cnt, magic_ext, len(diff_norm_lines)))
                    del diff_vuln_lines[:]
                    del diff_orig_lines[:]
                ...
                if diff_path == '/dev/null':
                    ...
                    process_flag = True
                    diff_cnt = 0
                    diff_file = re.split('/|\\\\', diff_path)[-1]
                ...
                if line.startswith('+++ '):
                    ...
                    diff_orig_lines.append(line.replace(
                        '<', '&lt;').replace('>', '&gt;'))
    ...
    def _normalize(self, patch, ext):
        ...
        if ext == common.FileExt.C or ext == common.FileExt.Java:
            ...
            patch = ''.join([c.group('noncomment') for c in common.shellscript_regex.finditer(
                patch) if c.group('noncomment')])
        elif ext == common.FileExt.Perl:
            patch = ''.join([c.group('noncomment') for c in common.perl_regex.finditer(
                patch) if c.group('noncomment')])
        elif ext == common.FileExt.PHP:
            patch = ''.join([c.group('noncomment') for c in common.php_regex.finditer(
                patch) if c.group('noncomment')])
            patch = ''.join([c.group('noncomment') for c in common.c_partial_comment_regex.finditer(
                patch) if c.group('noncomment')])
        elif ext == common.FileExt.Ruby:
            patch = ''.join([c.group('noncomment') for c in common.ruby_regex.finditer(
                patch) if c.group('noncomment')])
            patch = ''.join([c.group('noncomment') for c in common.ruby_partial_comment_regex.finditer(
                patch) if c.group('noncomment')])
    ...
    def _get_file_type(self, file_path):
        ...
        if file_type is None:
            ...
            main_type, sub_type = re.split('/|\\\\', file_type)

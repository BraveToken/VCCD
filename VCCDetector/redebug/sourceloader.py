...
class SourceLoader(object):
    ...
    def __init__(self):
        ...
        self._second_match_dict = defaultdict(list)
        self._nmatch = 0
    ...
    def traverse(self, source_path, patch):
        ...
        if os.path.isfile(source_path):
            ...
            if magic_type.startswith('text'):
                main_type, sub_type = re.split('/|\\\\', magic_type)
            ...
            for root, dirs, files in os.walk(source_path):
                for file in files:
                    ...
                    common.verbose_print('  [-] %s: %s' %
                                         (file_path, magic_type))
                    if magic_type.startswith('text'):
                        main_type, sub_type = re.split('/|\\\\', magic_type)
                        magic_ext = self._get_file_type(sub_type)
    ...
    def _process(self, source_path, magic_ext):
        ...
        if self._query_bloomfilter(source_norm_lines, magic_ext, source_path):
            source_norm_lines = re.split('\n', source_norm_lines)
            source_orig_lines = re.split('\n', source_orig_lines)
            self._source_list.append(common.SourceInfo(
                source_path, magic_ext, source_orig_lines, source_norm_lines))
            self._nsource += 1
    ...
    def _normalize(self, source, ext):
        ...
        if ext == common.FileExt.C or ext == common.FileExt.Java:
            ...
            source = ''.join([c.group('noncomment') for c in common.shellscript_regex.finditer(
                source) if c.group('noncomment')])
        elif ext == common.FileExt.Perl:
            source = ''.join([c.group('noncomment') for c in common.perl_regex.finditer(
                source) if c.group('noncomment')])
    ...
    def _query_bloomfilter(self, source_norm_lines, magic_ext, source_path):
        source_norm_lines = source_norm_lines.split()
        if len(source_norm_lines) < common.ngram_size:
            common.verbose_print('      - skipped (%d lines)' %
                                 len(source_norm_lines))
            return False

        self._source_set.add(source_path)
        ...
        for i in range(0, num_ngram):
            if num_ngram_processed > common.bloomfilter_size/common.min_mn_ratio:
                common.verbose_print(
                    '      - split Bloom filters (%d n-grams)' % num_ngram_processed)
                for patch_id in range(0, self._npatch):
                    if magic_ext == self._patch_list[patch_id].file_ext:
                        ...
                        if is_match:
                            ...
                            self._second_match_dict[self._nsource].append(
                                patch_id)
                            common.verbose_print(
                                '      - match (patch #%d : source #%d)' % (patch_id, self._nsource))
                            self._nmatch += 1
                num_ngram_processed = 0
                self._bit_vector.setall(0)
        ...
        for patch_id in range(0, self._npatch):
            if magic_ext == self._patch_list[patch_id].file_ext:
                ...
                if is_match:
                    is_vuln_source = True
                    self._match_dict[patch_id].append(self._nsource)
                    self._second_match_dict[self._nsource].append(patch_id)
                    common.verbose_print(
                        '      - match (patch #%d : source #%d)' % (patch_id, self._nsource))
                    self._nmatch += 1
    ...
    def second_match_items(self):
        return self._second_match_dict
